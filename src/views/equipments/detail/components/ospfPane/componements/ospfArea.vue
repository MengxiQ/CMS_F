<template>
  <div>
    ...
    <el-table
      v-if="list.areas"
      :data="isArray(list.areas.area)?list.areas.area:Array(list.areas.area)"
      highlight-current-row
    >
      <el-table-column type="expand">
        <template slot-scope="props">
          <div style="background: #f9f9f9; padding: 10px">
            <h3 class="label-h5">networks：
              <el-link type="primary" @click="createInstance('network_params',props.row)">新建网络</el-link>
            </h3>
            <!--  v-if="props.networks" // 如果区域没添加属性就False-->
            <el-table
              v-if="props.row.networks"
              :data="isArray(props.row.networks.network) ? props.row.networks.network:Array(props.row.networks.network)"
            ...
            <h3 class="label-h5">接口列表：
              <!--              <el-link type="primary">添加接口</el-link>-->
            </h3>
            <!--  v-if="props.networks" // 如果区域没添加属性就False-->
            <el-table
              v-if="props.interfaces"
              :data="isArray(props.row.interfaces.interface) ? props.row.interfaces.interface:Array(props.row.interfaces.interface)"
            >
...
export default {
  ...
  methods: {
    ...
    createInstance(param_name, row) {
      ...
      console.log('activeParam', this.activeParam)
    ...
    getList() {
      getOspfArea(this.ip).then(res => {
        this.area_params = res.params
        this.temp.processId = this.processId
      }).catch(error => {
        this.$message({type: 'error', message: error.response.data['msg']})
      })
      getOspfAreaNetwork(this.ip).then(res => {
